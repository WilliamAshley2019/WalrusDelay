 ...
 * and a simplified reverb effect, all contained within the processor to avoid external dependencies.
 */
...
private:
    ...
    // Low-Pass Filter (simplified from Filters.h)
    struct LowPassFilter
    {
        ...
        void update(float frequency)
        {
            ...
            {
                smoothedFreq.setTargetValue(juce::jlimit(200.0f, 4000.0f, frequency));
                float freq = smoothedFreq.getNextValue();
                float omega = 2.0f * juce::MathConstants<float>::pi * freq / sampleRate;
                alpha = sin(omega) / (2.0f * 0.5f); // Q = 0.5
                float cosOmega = cos(omega);
                a0 = 1.0f + alpha;
                b1 = -2.0f * cosOmega / a0;
                b2 = (1.0f - alpha) / a0;
                a0 = (1.0f - cosOmega) / (2.0f * a0);
                a1 = a0;
                float gain = a0 + a1 + a0;
                a0 /= gain;
                a1 /= gain;
